...
def main(search_object):
    ...
    file.truncate(0)
    # headers = "Name\tPrice\tSpecification\tOffers \n"
    # file.write(headers)
    i, count = 0, 0
    ...
    while count <= 500 or i < 2:
        try:
            ...
            for page in products:
                ...
                try:
                    singleProductName = str(nameOfProduct(page))
                except:
                    singleProductName = 'None'

                try:
                    singleProductPrice = str(priceOfProduct(page))
                except:
                    singleProductPrice = 'None'

                try:
                    singleProductSpecification = specificationOfProducts(page)

                except:
                    singleProductSpecification = 'None'

                try:
                    singleProductOffers = str(offersOnTheProduct(page))
                except:
                    singleProductOffers = 'None'

                try:
                    singleProductLink = ProductLink(page)
                except:
                ...
                if singleProductName == 'None' and singleProductPrice == 'None' and singleProductSpecification == 'None' and singleProductOffers == 'None':
                    ...
                    count = count+1
                    line = str(singleProductName) + '\t' + str(singleProductPrice) + '\t' + "  ".join(singleProductSpecification) + '\t' + str(singleProductOffers) + '\n'
                    file.write(line)
